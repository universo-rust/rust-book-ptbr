...
No **Capítulo 4**, você aprenderá sobre o sistema de ownership (propriedade) do Rust. O **Capítulo 5** discute structs e métodos. O **Capítulo 6** cobre enums, expressões `match` e as construções de controle de fluxo `if let` e `let...else`. Você usará structs e enums para criar tipos personalizados.
